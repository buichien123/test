...
const executeSQL = async (connection, sql, description) => {
  try {
    ...
    try {
      ...
      log.warning(`Batch execution error: ${batchError.message}`);
      log.warning("Trying to execute statement by statement...");
      ...
      for (let i = 0; i < sql.length; i++) {
        ...
        if (char === ";") {
          ...
          const hasOnDuplicateKeyUpdate = /ON\s+DUPLICATE\s+KEY\s+UPDATE/i.test(
            currentStatement
          );

          // Check if this is the end of an INSERT statement by looking for ON DUPLICATE KEY UPDATE pattern
          // and ensuring we're not in a string and parentheses are balanced (or we're after ON DUPLICATE KEY UPDATE)
          const isAfterOnDuplicate =
            hasOnDuplicateKeyUpdate &&
            /ON\s+DUPLICATE\s+KEY\s+UPDATE[^;]*;/.test(
              currentStatement.substring(
                Math.max(0, currentStatement.length - 200)
              )
            );

          // Check if we have a complete INSERT statement (has VALUES and ends with ON DUPLICATE KEY UPDATE or just semicolon)
          const hasInsertValues = /INSERT\s+INTO\s+\w+\s+[^;]*VALUES/i.test(
            currentStatement
          );
          const hasCompleteInsert =
            hasInsertValues && (hasOnDuplicateKeyUpdate || parenDepth === 0);

          // Check if next statement starts with INSERT INTO (to avoid merging statements)
          const nextChars = sql.substring(i + 1, i + 100).trim();
          const hasNextInsert = /^\s*INSERT\s+INTO/i.test(nextChars);
          const hasNextComment = /^\s*--/.test(nextChars);

          // End statement if:
          // 1. Not in string AND
          // 2. (Parentheses balanced OR we're after ON DUPLICATE KEY UPDATE) AND
          // 3. Next is not another INSERT (unless we have complete INSERT with ON DUPLICATE KEY UPDATE)
          const shouldEndStatement =
            !inString &&
            ((parenDepth === 0 && !hasNextInsert && !hasNextComment) ||
              (isAfterOnDuplicate && !hasNextInsert) ||
              (hasCompleteInsert && hasOnDuplicateKeyUpdate && !hasNextInsert));
            ...
            if (trimmed.length > 0) {
              ...
              if (
                ...
                if (statements.length === 18) {
                  ...
                  log.info(
                    `  Last 100 chars: ${cleaned.substring(
                      cleaned.length - 100
                    )}`
                  );
                }
      ...
      for (let i = 0; i < statements.length; i++) {
        ...
        try {
          ...
          const sqlToExecute = statement.endsWith(";")
            ? statement
            : statement + ";";
          ...
          if (i === 17) {
            ...
            log.info(
              `Last 100 chars: ${statement.substring(statement.length - 100)}`
            );
            log.info(`=== END DEBUG ===\n`);
          }
      ...
      if (errorCount === 0) {
        log.success(
          `${description} - Completed (${successCount} statements executed)`
        );
      } else {
        log.warning(
          `${description} - Completed with ${errorCount} warnings (${successCount} successful)`
        );
      }
      return true;
    }
...
const seedDatabase = async () => {
  ...
  try {
    ...
    if (!usersTableExists || !productsTableExists) {
      log.info(
        "Database structure not found, creating tables and seeding data..."
      );
    } else {
      ...
      if (hasSeederData) {
        ...
        log.info(
          "Skipping seeder. If you want to reset, drop the database first."
        );
        console.log("\n" + "-".repeat(60));
        ...
        log.info(
          "Database structure exists but seeder data not found, seeding data..."
        );
      }
    }
    ...
    if (!usersTableExists || !productsTableExists) {
      ...
      const createTablesSQL =
        cleanedSQL.match(/CREATE TABLE[^;]+;/gi)?.join("\n") || "";
      if (createTablesSQL) {
        await executeSQL(
          connection,
          createTablesSQL,
