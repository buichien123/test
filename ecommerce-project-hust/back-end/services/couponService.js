...
const getAllCoupons = async (filters = {}) => {
  ...
  const [countResult] = await pool.execute(
    status
      ? "SELECT COUNT(*) as total FROM coupons WHERE status = ?"
...
const validateCoupon = async (code, userId, totalAmount) => {
  ...
  if (coupon.min_purchase_amount && totalAmount < coupon.min_purchase_amount) {
    throw new Error(
      `Đơn hàng tối thiểu ${coupon.min_purchase_amount.toLocaleString(
        "vi-VN"
      )}đ để sử dụng mã này`
    );
  }
...
const useCoupon = async (couponId, userId, orderId, discountAmount, connection = null) => {
...
const createCoupon = async (couponData) => {
  ...
  if (!code || !discount_type || !discount_value || !start_date || !end_date) {
    throw new Error(
      "Mã giảm giá, loại giảm giá, giá trị giảm giá, ngày bắt đầu và ngày kết thúc là bắt buộc"
    );
  }
  ...
  const formatDateTime = (dateValue) => {
    ...
    // Convert ISO format or Date object to MySQL format
  ...
  const [coupons] = await pool.execute("SELECT * FROM coupons WHERE id = ?", [
    result.insertId,
  ]);
  return coupons[0];
};
...
const updateCoupon = async (id, couponData) => {
  ...
  const formattedStartDate = start_date ? formatDateTime(start_date) : null;
  const formattedEndDate = end_date ? formatDateTime(end_date) : null;

  // Validate date range if both dates are provided
  if (
    formattedStartDate &&
    formattedEndDate &&
    moment(formattedEndDate).isBefore(moment(formattedStartDate))
  ) {
    throw new Error("Ngày kết thúc phải sau ngày bắt đầu");
  }
