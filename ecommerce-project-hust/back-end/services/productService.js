...
const getProductById = async (id) => {
  ...
  // Get related products - first try from product_relations, then fallback to same category
  let [relatedProducts] = await pool.execute(
    `SELECT p.*, c.name as category_name
     FROM products p
     LEFT JOIN categories c ON p.category_id = c.id
     INNER JOIN product_relations pr ON p.id = pr.related_product_id
     WHERE pr.product_id = ? AND p.status = 'active' AND p.id != ?
     LIMIT 8`,
    [id, id]
  );

  // If no related products from relations, get products from same category
  if (relatedProducts.length === 0 && product.category_id) {
  ...
  // If still no products, get any active products (excluding current)
...
const updateProduct = async (id, productData) => {
  const { name, description, price, stock, image_url, category_id, status, variants, images } =
    productData;
    ...
    if (images !== undefined) {
      ...
      if (images && images.length > 0) {
        for (let i = 0; i < images.length; i++) {
          await connection.execute(
            "INSERT INTO product_images (product_id, image_url, is_primary, display_order) VALUES (?, ?, ?, ?)",
            [id, images[i].url, images[i].is_primary || i === 0, images[i].display_order || i]
          );
        }
      }
    }
