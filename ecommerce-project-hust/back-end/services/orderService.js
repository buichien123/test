...
const createOrder = async (userId, orderData) => {
  ...
  try {
    ...
    // Record coupon usage
    if (couponId) {
      await couponService.useCoupon(couponId, userId, orderId, discountAmount, connection);
    }
    ...
    try {
      ...
      // Don't throw error, order was already created successfully
    }
...
const getAllOrders = async (filters = {}) => {
  ...
  let query = `
    SELECT o.*, u.username, u.email, u.full_name, c.code as coupon_code,
    (SELECT COUNT(*) FROM order_items WHERE order_id = o.id) as item_count
    FROM orders o 
